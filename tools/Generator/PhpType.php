...
final readonly class PhpType
{
    ...
    /**
     * @param string|null $class   Fully qualified class name of an enum or model.
    ...
    /**
     * The native type declaration for reading, e.g. `string`, `array`, `PullZone`.
     *
    ...
    public function native(callable $alias, bool $forWriting = false): string
    {
        return match ($this->kind) {
            ...
            self::ENUM, self::MODEL => $alias($this->classOrFail()),
    ...
    /**
     * The PHPDoc type, e.g. `list<Hostname>` or `array<array-key, float>`.
     *
    ...
    public function classes(bool $forWriting = false): array
    {
        return match ($this->kind) {
            ...
            self::ENUM, self::MODEL => [$this->classOrFail()],
