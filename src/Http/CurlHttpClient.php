 ...
 * single cURL handle, so connections stay alive across requests, and it can be
 * shared by all API clients. On PHP 8.5+ it additionally shares DNS, connection
 * and TLS session caches across requests of the same worker process through a
 * persistent share handle.
 */
final class CurlHttpClient implements HttpClient
{
    /**
     * Seconds a transfer without a total time limit may stall (below 1 byte/s)
    ...
    public function send(Request $request): Response
    {
        ...
        $options = [
            ...
            \CURLOPT_WRITEFUNCTION => static function (CurlHandle $handle, string $chunk) use ($sink, &$body): int {
                $status = curl_getinfo($handle, \CURLINFO_RESPONSE_CODE);

                if ($sink === null || !\is_int($status) || $status < 200 || $status >= 300) {
                    $body .= $chunk;

                    return \strlen($chunk);
                }

                return self::writeFully($sink, $chunk) ? \strlen($chunk) : 0;
            },
        ];
        ...
        if ($timeout <= 0.0) {
            // No total limit (e.g. large uploads), but never hang on a dead connection.
    ...
    private function applyBody(Request $request, array &$options): void
    {
        ...
        if ($body instanceof Stream) {
            ...
            if ($body->size !== null) {
                $options[\CURLOPT_INFILESIZE] = $body->size;
            }
        ...
        if (!$request->method->isIdempotent() || $request->method === Method::Put) {
            // Send an explicit "Content-Length: 0"; some endpoints reject a
            // body-less POST without it (411 Length Required).
    ...
    private static function writeFully(Stream $sink, string $chunk): bool
    {
        while ($chunk !== '') {
            $written = fwrite($sink->resource, $chunk);
    ...
    /**
     * Drop the query string, which may carry signed tokens, from error messages.
     */
